...
NS_LOG_COMPONENT_DEFINE ("test");
  ...
  Ptr<OutputStreamWrapper> stream = asciiTraceHelper.CreateFileStream ("new-res-one-device.txt");
...
Simulator::Stop (Seconds (stopTime));
  AnimationInterface anim ("new-res-one-device.xml");
  Simulator::Run ();
  
  flowMonitor->SerializeToXmlFile("new-res-one-device-flow.xml",true,true);
  Simulator::Destroy ();
