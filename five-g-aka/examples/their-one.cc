...
NS_LOG_COMPONENT_DEFINE ("test");
  ...
  Ptr<OutputStreamWrapper> stream = asciiTraceHelper.CreateFileStream ("their-one-device.txt");
...
static void RxofSE (Ptr<Socket> socket, Address hnadd,Address ueadd,Ptr<const Packet> packet, const Address &from)
{
    ...
    if(key == 5)
    {
      ...
      hashRes(value,newValue);
       bool checkRes = se.verifyResT(newValue);
...
static void RxofUE (Ptr<Socket> socket, Address add,Ptr<const Packet> packet, const Address &from)
{
    ...
    if(key == 4)
    {
      ...
      bool macVerification = ue.calculation(R1,decryptKey,1);
      if(macVerification)
      {
...
Simulator::Stop (Seconds (stopTime));
  AnimationInterface anim ("their-one-device.xml");
  Simulator::Run ();
  
  flowMonitor->SerializeToXmlFile("their-one-device-flow.xml",true,true);
  Simulator::Destroy ();
