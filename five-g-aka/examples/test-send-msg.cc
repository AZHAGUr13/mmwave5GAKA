...
NS_LOG_COMPONENT_DEFINE ("test");
  ...
  Ptr<OutputStreamWrapper> stream = asciiTraceHelper.CreateFileStream ("new-one-device.txt");
...
Simulator::Stop (Seconds (stopTime));
  AnimationInterface anim ("new-one-device.xml");
  Simulator::Run ();
  
  flowMonitor->SerializeToXmlFile("new-one-device-flow.xml",true,true);
  Simulator::Destroy ();
