...
Ptr<OutputStreamWrapper> stream = asciiTraceHelper.CreateFileStream ("new-random-device.txt");
Ptr<OutputStreamWrapper> stream1 = asciiTraceHelper.CreateFileStream ("new-random-assign.txt");
...
static void assignNodes(uint i,uint j,uint k)
{
  *stream1->GetStream ()<<i<<"\t"<<j<<"\t"<<k<<"\n";
}
      ...
      if (isTrue)
        {
          ...
          socket->Send (packet);
          // socket->Close();
          std::cout << "#######\t Msg with " << 3 << " send HN "<<aue.getH(token1)<<"####\n";
        }
      else
        {
          //ABORT
...
ipv4h.SetBase ("10.1.1.0", "255.255.255.0","0.0.0.1");
  ...
  for(unsigned int i=0; i< aue.getSize();i++)
  {
    assignNodes(i,aue.getS(i),aue.getH(i));
  }
  *stream1->GetStream()<<"\nAttack\n UE\t"<<aue.getUEAttack()<<"\tSN"<<aue.getSNAttack();
  ...
  Simulator::ScheduleWithContext (SESocket[aue.getS(i)]->GetNode ()->GetId (), Seconds (rand()%10), &sendToUE,
...
Ptr<FlowMonitor> flowMonitor;
 ...
 flowMonitor = flowHelper.InstallAll();
  ...
  AnimationInterface anim ("new-random-device.xml");
  Simulator::Run ();
  
  flowMonitor->SerializeToXmlFile("new-random-device-flow.xml",true,true);
  Simulator::Destroy ();
