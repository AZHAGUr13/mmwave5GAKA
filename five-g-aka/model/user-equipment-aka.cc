...
NS_OBJECT_ENSURE_REGISTERED (UserEqupi);
...
UserEqupi::setString(u8 a[],u8 suciSN[])
{
 ...
 //std::cout<<"\nFresh Token At UE:\n";
  ...
  //  std::cout<<"\nHRES Token At UE from AVC:\n";
    ...
    {
      suciSN[i+16] = resUE[i];
  //    std::cout<<int(suciSN[i+16])<<" ";
    }
   // std::cout<<"\nSUCI in UE\n";

}
void 
UserEqupi::setRandom(u8 a[],u8 suciSN[])
{
  for(unsigned int i=0;i<16;i++)
      R1[i] = a[i];
...
bool 
UserEqupi::calculation(u8 xamc[],u8 resUE[],uint)
{
  // calculate fresh key from two random numbers
 // unsigned int k = R1[15];
  u8 mac_u[8],amf[2];
  u8 fresh[16],resUE1[32];
...
/* K ausf Token calculation */
 ...
 //     std::cout<<int(ausf[i])<<" ";
    ...
    oldRes(FC,p0,l0,fresh,l1,key,l2,resUE1);
    for(unsigned int j=0; j<32; j++)
    {
      resUE[j]= resUE1[j];
      std::cout<<int(resUE[j])<<" ";
    }
/* K seaf Token Calculation */
   FC = 0x6c;
  calculateKseaf(FC,p0,l0,ausf,ek);
...
TypeId 
UserEqupi::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::UserEqupi")
    .SetParent<Object> ()
    .AddConstructor<Node> ()
    ;
    return tid;
