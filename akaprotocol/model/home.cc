...
/* ... */
NS_LOG_COMPONENT_DEFINE("oldhome");
...
bool oldhome::calculation(u8 input[],u8 output[])
{
...
/* Hres Token calculation */
    ...
    oldRes(FC,p0,l0,r2,l1,key,l2,hres);
   // std::cout<<"\nHRES Token At HN:\n";
    for(unsigned int i=0;i<32;i++)
    {
      if (int(hres[i])!= int(input[i]))
      {
        return false;
      }
    }
...
/* K seaf Token Calculation */
   FC = 0x6c;
  calculateKseaf(FC,p0,l0,ausf,output);
 // std::cout<<"\nSEAF Token At HN:\n";
    for(unsigned int i=0;i<32;i++)
    {
  //    std::cout<<int(ek[i])<<" ";
...
bool oldhome::setString(u8 input[],u8 output[])
{
      for (unsigned int i=0;i<16;i++)
      {
        if(supi[i]!=input[i])
    ...
    u8 l2 = 0x80;
    u8 res[32];
    oldRes(FC,p0,l0,r2,l1,key,l2,res);
    hashRes(res,hres);
       //   std::cout<<"\n##########IN \n";
         for(unsigned int i=0;i<8;i++)
         {
...
TypeId 
oldhome::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::oldhome")
    .SetParent<Object> ()
    .AddConstructor<Node> ()
    ;
    return tid;
