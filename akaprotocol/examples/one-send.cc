...
NS_LOG_COMPONENT_DEFINE ("test");
  ...
  Ptr<OutputStreamWrapper> stream = asciiTraceHelper.CreateFileStream ("old-one-device.txt");
...
Simulator::Stop (Seconds (stopTime));
  AnimationInterface anim ("old-one-device.xml");
  Simulator::Run ();

  flowMonitor->SerializeToXmlFile("old-one-device-flow.xml",true,true);
  Simulator::Destroy ();
