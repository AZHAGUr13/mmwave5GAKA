...
Ptr<OutputStreamWrapper> stream = asciiTraceHelper.CreateFileStream ("old-random-device.txt");
  ...
  if (key == 1)
    {
     ...
     // aue.getServingN (aue.getS(token1))->getR1(R1,token1);
      ...
      socket->Send (packet);
      //   socket->Close();
      std::cout << "#######\t Msg with " << 2 << " send SE by "<<aue.getS(token1)<<"###\n";
    }
  if (key == 3)
    {
     ...
     // std::cout << "\n Res verification " ;
      ...
      aue.getServingN (aue.getS(token1))->setHRES(value,token1);
          ...
          socket->Send (packet);
          std::cout << "#######\t Msg with " << 4 << " send SE by "<<aue.getS(token1)<<"###\n";
  ...
  if (key == 5)
    {
      ...
      if(checkRes > 0)
      {
        ...
        socket->Connect (hnadd[aue.getS(token1)][aue.getH(token1)]);
        socket->Send (packet);
        std::cout<<"#######\t Msg with "<< 6 <<" send SE by "<<aue.getS(token1)<<"###\n";
      }
      else
      {
...
CwndChange (kk, from,"HN",token, key, uint32_t (17), value);
     ...
     if( key == 6 )
    {
      ...
      socket->Send (packet);
     // socket->Close();
      	std::cout<<"#######\t Msg with "<< 7 <<" send HN "<<aue.getH(token1)<<"####\n";
      }
      else
      {
        /* code */
...
Ptr<FlowMonitor> flowMonitor;
 ...
 flowMonitor = flowHelper.InstallAll();
  ...
  AnimationInterface anim ("old-random-device.xml");
  Simulator::Run ();
  
  flowMonitor->SerializeToXmlFile("old-random-device-flow.xml",true,true);
  Simulator::Destroy ();
