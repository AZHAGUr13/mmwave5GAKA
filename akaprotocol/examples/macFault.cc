...
NS_LOG_COMPONENT_DEFINE ("test");
  ...
  Ptr<OutputStreamWrapper> stream = asciiTraceHelper.CreateFileStream ("old-mac-one-device.txt");
...
static void RxofSE (Ptr<Socket> socket, Address hnadd,Address ueadd,Ptr<const Packet> packet, const Address &from)
{
    ...
    {
       CwndChange(kk,from,key,uint32_t(32),value);
       std::cout<<"Final ,essage received \n";
...
static void RxofHN (Ptr<Socket> socket, Address add,Ptr<const Packet> packet, const Address &from)
{
    ...
    if(key == 2 )
    {
     ...
     {
         std::cout<<"Failed to validate in HN\n";
     }
...
Simulator::Stop (Seconds (stopTime));
  AnimationInterface anim ("old-mac-one-device.xml");
  Simulator::Run ();

  flowMonitor->SerializeToXmlFile("old-mac-one-device-flow.xml",true,true);
  Simulator::Destroy ();
